...
// Added a more visible default border (border-gray-400) and a distinct background (bg-gray-50)
...
export default function TestsCreation() {
  ...
  // All logic functions (isStepValid, validateStep, etc.) remain unchanged.
  const isStepValid = () => {
    if (step === 0) return !!(questionPdfFile || questionPdfUrl);
    if (step === 1) {
    ...
    if (step === 2) {
      return !(!title || !description || !type || !testMode || !subject || !scheduledDate || !duration || duration <= 0 || !questionCount || questionCount <= 0);
    }
    return true;
  };
  const validateStep = () => {
    let errs = {};
    if (step === 0) { if (!questionPdfFile && !questionPdfUrl) errs.pdfUrl = "Questions PDF is required."; }
    if (step === 1) {
      if (!questionCount || questionCount <= 0) errs.questionCount = "Question count required.";
      if (!answerKey || Object.keys(answerKey).length !== Number(questionCount)) errs.answerKey = "Answer key count must match question count.";
    }
    if (step === 2) {
      ...
      if (new Date(scheduledDate) < new Date()) errs.scheduledDate = "Date cannot be in the past.";
  ...
  useEffect(() => {
    if (errors.answerKey && answerKey && Object.keys(answerKey).length === Number(questionCount)) {
      setErrors(prev => { const newErrs = { ...prev }; delete newErrs.answerKey; return newErrs; });
    }
    if (errors.questionCount && questionCount && Number(questionCount) > 0) {
      setErrors(prev => { const newErrs = { ...prev }; delete newErrs.questionCount; return newErrs; });
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (questionPdfFile && !questionPdfUrl) uploadedQuestionUrl = await uploadPdfAndGetUrl(questionPdfFile);
      if (!uploadedQuestionUrl) { setErrors({ pdfUrl: "Failed to upload Questions PDF." }); setPublishing(false); return; }
      setQuestionPdfUrl(uploadedQuestionUrl);
      let uploadedAnswersUrl = answersPdfUrl;
      if (answersPdfFile && !answersPdfUrl) uploadedAnswersUrl = await uploadPdfAndGetUrl(answersPdfFile);
      setAnswersPdfUrl(uploadedAnswersUrl);
      const payload = { title, description, syllabus, type, testMode, scheduledDate, duration: Number(duration), questionCount: Number(questionCount), isPublic, pdfUrl: uploadedQuestionUrl, answerKey, subject, answersPdfUrl: uploadedAnswersUrl || "" };
      const res = await axios.post("/test", payload);
      setShareLink(`${window.location.origin}/tests/${res.data.test.link}/take`);
      setStep(Steps.length);
    } catch { setErrors({ publish: "Failed to create test. Please try again." }); } finally { setPublishing(false); }
  };
  const copyLink = () => {
  ...
  const PreviewCard = () => (
    <div className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50 p-6 rounded-2xl shadow-sm space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
          ...
          <div className="col-span-2"><p className="text-xs text-gray-500">Scheduled</p><p className="font-semibold">{new Date(scheduledDate).toLocaleString()}</p></div>
      </div>
      <div className="flex items-center gap-4 text-sm">
        <DocumentCheckIcon className="h-5 w-5 text-green-600 flex-shrink-0" />
        <a href={questionPdfUrl} target="_blank" rel="noopener noreferrer" className="font-semibold text-indigo-600 hover:underline">
          Questions PDF
        </a>
        {answersPdfUrl && (
          <>
            <span className="text-gray-300 dark:text-gray-600">|</span>
            <a href={answersPdfUrl} target="_blank" rel="noopener noreferrer" className="font-semibold text-indigo-600 hover:underline">
          ...
          {Steps.map((s, idx) => (
            <React.Fragment key={idx}>
              <div className="flex items-center flex-col">
                <div
                  className={`w-10 h-10 flex items-center justify-center rounded-full font-bold text-lg transition-all duration-300 ${idx === step ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/30 scale-110" : idx < step ? "bg-green-600 text-white" : "bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400"}`}
                >
                  {React.createElement(s.icon, { className: "h-5 w-5" })}
                </div>
                <span className={`mt-2 text-xs font-bold text-center ${idx === step ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-500'}`}>{s.name}</span>
              </div>
              {idx < Steps.length - 1 && (
        ...
        <div className="min-h-[400px]">
            {step === 0 && (
              <StepWrapper title="Step 1: Upload Question Paper" icon={CloudArrowUpIcon}>
                <PDFUpload
                  ...
                  onUpload={(file, url) => { setQuestionPdfFile(file); setQuestionPdfUrl(url); }}
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <FormInput
                      ...
                      className="w-24" // Drastically reduced width
                    />
                  ...
                  <div className="pt-6 border-t border-gray-200 dark:border-gray-700/50">
                    <PDFUpload
                      ...
                      onUpload={(file, url) => { setAnswersPdfFile(file); setAnswersPdfUrl(url); }}
              ...
              <StepWrapper title="Step 3: Configure Test Details" icon={WrenchScrewdriverIcon}>
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <FormTextarea
                      value={syllabus} onChange={(e) => setSyllabus(e.target.value)} rows={6}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                      ...
                      <FormInput type="datetime-local" value={scheduledDate}
                    ...
                    <div>
                      ...
                      <FormInput type="number" value={duration} onChange={(e) => setDuration(e.target.value)} />
                      <ErrorMessage name="duration" />
                    </div>
                    <div>
                      <FormLabel># of Questions</FormLabel>
                      <FormInput type="number" value={questionCount} onChange={(e) => setQuestionCount(e.target.value)} />
                      <ErrorMessage name="questionCount" />
                    </div>
                  </div>
            ...
            {step === Steps.length && (
              <div className="text-center bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50 p-8 rounded-2xl shadow-sm">
                ...
                <p className="mt-1 text-gray-600 dark:text-gray-400">Share this link with participants to begin the test.</p>
                <div className="mt-6 max-w-lg mx-auto">
                    <div className="flex rounded-lg shadow-sm">
                        ...
                        <input type="text" readOnly value={shareLink} className="flex-1 block w-full min-w-0 rounded-none bg-gray-50 dark:bg-gray-900/50 border-gray-300 dark:border-gray-600 focus:ring-indigo-500 focus:border-indigo-500"/>
                        <button
                            onClick={copyLink}
