...
def extract_candidate_keys(text: str, max_q: int):
    ...
    if clean_tbl:
        candidates['clean_table'] = {str(int(num)): ans.upper() for num, ans in clean_tbl}

    # --- Strategy 2: Sequential List Format: Ans) d ---
    seq_list = re.findall(r'Ans\)\s*([a-dA-D])', text)
    if seq_list:
        candidates['sequential_list'] = {str(i+1): ans.upper() for i, ans in enumerate(seq_list)}
...
def select_best_key(candidates, max_q):
    ...
    priority = ['clean_table', 'token_pair', 'sequential_list']
    results.sort(key=lambda x: (x[0], x[1], priority.index(x[2]) if x[2] in priority else 99))
    return results[0][3]
...
if __name__ == '__main__':
    port = int(os.environ.get("PORT", 8001))  # default 8001 locally
